...
async function generateMorningPost(topicIndex = 0, alreadyGenerated = []) {
  ...
  const searchResult = await searchWeb(`${topic} 40代 50代 女性 頭痛 最新 研究 医師`)
  const searchSection = searchResult ? `
【最新のネット情報・研究データ（自然に投稿へ盛り込んでください）】
${searchResult}
` : ""
...
- 検索で得た医療情報・研究データがあれば、権威ある事実として自然に1つ盛り込む

【返信投稿のルール】
- 構成：①なぜうまくいかないか（根本原因・メカニズム）→ ②今日からできる行動リスト → ③締めの一言
- 根本原因は「研究では〜」「医師によると〜」など信頼感のある表現を使ってもよい
...
` : ""
  ...
  const lunchTopics = [
    ...
    "昼休みに外に出ると頭痛が減る理由（光・セロトニンの話）",
    "更年期と昼間の頭痛の関係・ホルモンと頭痛",
    "コーヒーで頭痛が治る人・悪化する人の違い",
  ]
  ...
  const afternoonTopics = [
    ...
    "長時間同じ姿勢でいると起こる体への影響",
    "肩こりをほぐすより先にやるべきこと",
    "夕方の頭痛薬が睡眠の質を下げる意外な理由",
    "週末に頭痛が出やすい人が知るべき「休日頭痛」の正体",
    "夕方に目がショボショボする人の頭痛リスク",
  ]
...
- 2行目：根本原因または医療的な事実を一言で（「実は〜」「研究では〜」など自然に）
- 3行目：今日すぐできる行動を一言で
- ネット検索の情報は、権威ある事実として自然に1つだけ盛り込む（出典名は書かない）
...
- 使える型：①「頭痛が出る人・出ない人の違いは〇〇」②「〇〇してるなら今すぐやめて」③「これを知らずに過ごすと損です」
- デスクワーク・昼休みなど具体的なシーンに落とし込む
- 医療的・研究的な事実を自然に入れると信頼感が増す
【絶対にやってはいけないこと】
...
- 検索で得た最新情報・研究データを「実は〜」「研究では〜」と一言自然に入れると効果的
【絶対にやってはいけないこと】
